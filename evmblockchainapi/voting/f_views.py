...
def check_voter(self, inputList, inputRecord):
    ...
    for record in inputList:
        if prevHash != record["prev_hash"]:
            found = True
            break
        elif record["elec_id"] != inputRecord["elec_id"]:
            continue
...
class CheckNom(APIView):
    ...
    def post(self, request, election_id, voter_id, format=None):
        ...
        if request.POST:
            ...
            #     print('Value: ', value)
            # print("4-------------------------------------")
            # print(type(list(next(iter(myDict.keys())))))
            ...
            # i = 0
            # for elem in got_list:
            #     print(i,elem)
            #     ddccii = dict(elem)
            #     print(i,dict(elem))
            #     print(ddccii['name'])
            #     #----------------------------- elec_id, voter_id, nom_data
            #     elec_id =

            #     i = i + 1
            # print("END-------------------------------------")

        # add_nom_block(election_id,voter_id,nom_data)
        return Response({'success': True})

 #    """
 #    Listclass CheckNom(APIView):
 # all snippets, or create a new snippet.
 #    """
